...
class CutWord:
    ...
    def _clean_data(self):
        ...
        temp_comment = re.sub(r"[%s]+" % hanzi.punctuation, " ", temp_comment)  # 处理中文标点
        temp_comment = re.sub(r"[%s]+" % string.punctuation, " ", temp_comment)  # 处理英文标点
    ...
    def _filter(self):
        self.comment = re.sub(r"[%s]+" % hanzi.punctuation, " ", self.comment)
        self.comment = re.sub(r"[%s]+" % string.punctuation.decode('unicode-escape'), " ", self.comment)
